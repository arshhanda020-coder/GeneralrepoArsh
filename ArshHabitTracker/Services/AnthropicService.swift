...
actor AnthropicService {
    ...
    func suggestion(for statusContext: String) async throws -> String {
        ...
        let body: [String: Any] = [
            ...
            "max_tokens": 300,
            "thinking": ["type": "adaptive"],
            "system": "You are a proactive assistant for ArshHabitTracker, a personal habit/skill/project tracker. Given the user's current status, suggest exactly ONE short, specific, encouraging next action. One or two sentences, no preamble, no lists.",
    ...
    You are the in-app copilot for ArshHabitTracker — a personal app for habits, skills, projects, and news. \
    You can both answer questions and take actions using your tools: check today's status, mark habits done \
    or not done, log skill practice sessions, add tasks to projects, and fetch cached news headlines by topic \
    (finance, accounting, economics, ai, or all). When the user asks you to do something ("mark X done", \
    "log a session for Y", "add a task to Z"), call the matching tool rather than just describing what you'd do. \
    Be concise — replies may be read aloud.
    """
