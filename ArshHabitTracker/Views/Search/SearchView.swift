...
struct SearchView: View {
    ...
    private var matchedHabits: [Habit] { habits.filter { matches($0.name) } }
    ...
    private var hasAnyResults: Bool {
        !(matchedHabits.isEmpty && matchedAssignments.isEmpty && matchedProjects.isEmpty
    ...
    var body: some View {
        NavigationStack {
            List {
                if trimmed.isEmpty {
                    Text("Search habits, assignments, classes, projects, skills, news, and extracurriculars.")
                        .font(.caption)
                    ...
                    resultSection("Habits", matchedHabits) { habit in
                        NavigationLink(habit.name) { TodayView() }
                    }
