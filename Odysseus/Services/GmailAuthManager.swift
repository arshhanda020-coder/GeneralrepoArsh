...
final class GmailAuthManager: NSObject, ObservableObject {
    ...
    /// Reflects whichever account is currently active (GmailAccountStore) —
    /// re-check via refreshConnectionStatus() after switching accounts.
    @Published var isConnected = false
    @Published var statusMessage: String?

    private var session: ASWebAuthenticationSession?
    private var codeVerifier: String?
    private var connectingAccountID: String?
    ...
    /// Re-reads whether the currently active GmailAccountStore account has a
    /// live connection — call after switching the active account.
    func refreshConnectionStatus() {
        guard let accountID = GmailAccountStore.activeAccountID else {
            isConnected = false
            return
        }
        isConnected = KeychainService.shared.loadGmailRefreshToken(accountID: accountID) != nil
    }
    ...
    func connect(accountID: String) {
        ...
        session = ASWebAuthenticationSession(url: authURL, callbackURLScheme: scheme) { [weak self] callbackURL, error in
            ...
            Task { @MainActor in
                await self.handleCallback(callbackURL, error: error, clientID: clientID, redirectURI: redirectURI, accountID: accountID)
            }
        }
    ...
    func disconnect(accountID: String) {
        KeychainService.shared.deleteGmailRefreshToken(accountID: accountID)
        refreshConnectionStatus()
    }

    private func handleCallback(_ callbackURL: URL?, error: Error?, clientID: String, redirectURI: String, accountID: String) async {
        if let error {
        ...
        await exchangeCode(code, clientID: clientID, redirectURI: redirectURI, accountID: accountID)
    }

    private func exchangeCode(_ code: String, clientID: String, redirectURI: String, accountID: String) async {
        ...
        do {
            ...
            if let refreshToken = json["refresh_token"] as? String {
                KeychainService.shared.saveGmailRefreshToken(refreshToken, accountID: accountID)
            }
            if let accessToken = json["access_token"] as? String {
                KeychainService.shared.saveGmailAccessToken(accessToken, expiresIn: json["expires_in"] as? Int ?? 3600, accountID: accountID)
            }
            let connected = KeychainService.shared.loadGmailRefreshToken(accountID: accountID) != nil
            if accountID == GmailAccountStore.activeAccountID {
    ...
    /// Returns a live access token for the given account, refreshing via its
    /// stored refresh token if needed.
    func validAccessToken(accountID: String) async -> String? {
        if let token = KeychainService.shared.loadGmailAccessTokenIfValid(accountID: accountID) {
            return token
        }
        guard let clientID, let refreshToken = KeychainService.shared.loadGmailRefreshToken(accountID: accountID) else { return nil }
        let params: [String: String] = [
            "client_id": clientID,
