...
//
//  HealthKit only exists on iOS/iPadOS (and Mac Catalyst) — a native macOS
//  target has no HealthKit framework at all — so the real implementation is
//  iOS-only below, with a same-API stub for macOS so SettingsView/HealthView
