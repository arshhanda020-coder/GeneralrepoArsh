...
actor OpenAIService: AIProviderService {
    ...
    today's status, look up the user's freeform notes (get_notes — covers notes pinned to Today, attached to a \
    project, a school topic/class, or a subagent, as well as standalone ones in the Notes hub; search by keyword \
    or by where a note is attached, e.g. a class or topic name) whenever they ask about something they wrote down \
    or need a refresher on class notes, log skill practice sessions, add/edit/delete tasks on projects, add/edit/delete extracurricular \
