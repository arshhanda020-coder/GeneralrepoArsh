...
struct BodyProgressView: View {
    ...
    @State private var sex: String = HealthProfile.sex ?? "Prefer not to say"
    @State private var activityLevel: ActivityLevel = ActivityLevel(rawValue: HealthProfile.activityLevel ?? "") ?? .lightlyActive
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(spacing: 10) {
                ...
                HStack {
                    ...
                    .pickerStyle(.menu)
                    .onChange(of: sex) { _, value in HealthProfile.sex = value }
                }
                HStack {
                    Text("Activity level")
                    Spacer()
                    ...
                    .pickerStyle(.menu)
                    .onChange(of: activityLevel) { _, value in HealthProfile.activityLevel = value.rawValue }
                }
            }
            .padding(12)
            .glassPanel(cornerRadius: 10)
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 10) {
                    ForEach(entries) { entry in
                        VStack(spacing: 4) {
                            if let data = entry.photoData, let uiImage = PlatformImage(data: data) {
                                Image(platformImage: uiImage)
                                    .resizable()
                                    .scaledToFill()
