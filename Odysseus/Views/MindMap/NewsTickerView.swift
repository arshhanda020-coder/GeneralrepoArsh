    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if topItems.isEmpty {
                ...
                // Wrapped in a GeometryReader specifically because a
                // GeometryReader always reports whatever size ITS OWN parent
                // proposes, regardless of its children's intrinsic size —
                // that's what actually stops the two cardRows' huge combined
                // width (thousands of points, by design, for the loop) from
                // leaking upward and distorting the whole screen's measured
                // size. .clipped() alone only crops what's drawn; it doesn't
                // change what a view reports as its ideal size to its parent.
                GeometryReader { outerGeo in
                    ...
                    .frame(width: outerGeo.size.width, height: 78, alignment: .leading)
                    .clipped()
                }
                .frame(height: 78)
                .padding(.bottom, 12)
